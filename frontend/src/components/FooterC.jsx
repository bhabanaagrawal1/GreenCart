      ...
      <div className='w-full h-[80vh] bg-gray-100 overflow-hidden'>
        <div className='w-full h-[50%] flex justify-around items-center'>
              ...
              <div className='flex justify-start gap-3'>
                {icons.map((item,index) =>(
                   <NavLink key={index} to={links[index]}>
                      <div className="w-14 h-14 rounded-full shadow-sm bg-white flex justify-center items-center text-xl hover:text-[24px]">{item}</div>
                   </NavLink>
                ))}
              </div>
            </div>
          ...
          {heading.map((item,index)=>(
            ...
            <div>
              <h1 className='text-[22px]'>{item}</h1>
              {footer[index + 1].map((itm,idx)=>(
                <h3 key={idx} className='text-[14px] p-1.75 text-black-600 hover:cursor-pointer hover:scale-102 hover:font-semibold hover:underline hover:underline-offset-2'>{itm}</h3>
              ))}
            </div>
          </div>
          ))}
